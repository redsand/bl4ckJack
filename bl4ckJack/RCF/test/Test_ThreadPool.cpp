...
namespace Test_ThreadManager {
    ...
    // RCF_CHECK() is currently not thread safe.
    RCF::Mutex rcfCheckMutex;
...
int test_main(int argc, char **argv)
{
    ...
    util::CommandLineOption<unsigned int> clientReps("clientReps", 50, "how many iterations an individual client goes through");
    ...
    for (std::size_t i=0; i<RCF::getTransportFactories().size(); ++i)
    {
        ...
        for (int j=0; j<2; ++j)
        {
            ...
            {
                threadPoolPtr.reset(
                    new RCF::ThreadPool(concurrencyCount+1));
            }
            else
            {
                threadPoolPtr.reset(
                    new RCF::ThreadPool(1,200));
            }
    ...
    for (std::size_t i=0; i<RCF::getTransportFactories().size(); ++i)
    {
        ...
        RCF::ThreadPoolPtr threadPoolPtr( new RCF::ThreadPool(
            ThreadCountInit, 
            ThreadCountMax, 
            "RCF Thread Pool",
            ReserveLastThread, 
        ...
        for (std::size_t j=0; j<3; ++j)
        {
            ...
            if (ReserveLastThread)
            {
                ...
                RCF_CHECK_EQ(errorsBusy + errorsOk , clients.size())
            ...
            else
            {
                ...
                RCF_CHECK_EQ(errorsBusy , 0)(errors);
                RCF_CHECK_LT(errorsOk , clients.size())(j)(errors);
            }
