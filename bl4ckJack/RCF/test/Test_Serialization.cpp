...
#if defined(RCF_USE_HASH_MAP) && defined(__GNUC__)
...
namespace Test_Serialization {
    ...
    void testAtomic()
    {
        ...
        RCF_CHECK_EQ(b2 , b1 );
        RCF_CHECK_EQ(n2 , n1 );
        RCF_CHECK_EQ(ch2 , ch1 );
        
        //RCF_CHECK_CLOSE(f2, f1, 1.1 );
        //RCF_CHECK_CLOSE(d2, d1, 1.1 );

        RCF_CHECK_LT( (f2-f1)*(f2-f1) , 0.1 );
        RCF_CHECK_LT( (d2-d1)*(d2-d1) , 0.1 );
    ...
    void testEmptyComposite()
    {
        ...
        RCF_CHECK_EQ(pe3 , NULL );
        RCF_CHECK( pe4 != NULL );
        RCF_CHECK( v2.empty() );
        RCF_CHECK_EQ(pv3 , NULL );
        RCF_CHECK( pv4 != NULL );
    ...
    template<typename IStream, typename OStream, typename Array>
    void testSafeArrays()
    {
        Array x;
        Array y;
    ...
    template<typename IStream, typename OStream, typename SharedPtr>
    void testSharedPtr()
    {
        int *pn = new int;
        *pn = 2;
        ...
        std::istringstream is(os.str());
        IStream istr(is);
        istr >> pn >> spn1 >> spn2 >> spn3 >> spn4;

        RCF_CHECK( pn != NULL );
        RCF_CHECK_EQ(pn == spn1.get() && pn == spn2.get() && pn , spn3.get() );
        RCF_CHECK_EQ(spn1.use_count() , 3 );
        RCF_CHECK_EQ(spn4.get() , NULL );
    }

    template<typename IStream, typename OStream, typename Tuple>
    void testTuple()
    {
        ...
        RCF_CHECK(t2 == u2 );
#else
        RCF_CHECK( 0 && "tuple serialization not working on vc6");
#endif
    ...
    void testTr1()
    {
        testSharedPtr<IStream, OStream, std::tr1::shared_ptr<int> >();
        testTuple<IStream,OStream, std::tr1::tuple<int, std::string, int, std::string> >();
        testSafeArrays<IStream, OStream, std::tr1::array<int, 10> >();

        std::tr1::unordered_map<std::string, int> um1, um2;
        std::tr1::unordered_multimap<std::string, int> umm1, umm2;

        std::tr1::unordered_set<std::string> us1, us2;
        std::tr1::unordered_multiset<std::string> ums1, ums2;

        um1["four"] = 4;
        um1["five"] = 5;

        umm1.insert( std::make_pair("six", 6) );
        umm1.insert( std::make_pair("six", 66) );
        umm1.insert( std::make_pair("seven", 7) );
        ...
        std::ostringstream os;
        OStream ostr(os);
        ostr << um1 << umm1 << us1 << ums1;

        std::istringstream is(os.str());
        IStream istr(is);
        istr >> um2 >> umm2 >> us2 >> ums2;

        std::set< std::pair<std::string, int> > um1AsSet(um1.begin(), um1.end());
        std::set< std::pair<std::string, int> > um2AsSet(um2.begin(), um2.end());
        RCF_CHECK(um1AsSet == um2AsSet);

        std::multiset< std::pair<std::string, int> > umm1AsSet(umm1.begin(), umm1.end());
        std::multiset< std::pair<std::string, int> > umm2AsSet(umm2.begin(), umm2.end());
        RCF_CHECK(umm1AsSet == umm2AsSet);

        std::set< std::string > us1AsSet(us1.begin(), us1.end());
        std::set< std::string > us2AsSet(us2.begin(), us2.end());
        RCF_CHECK(us1AsSet == us2AsSet);

        std::multiset< std::string > ums1AsSet(ums1.begin(), ums1.end());
        std::multiset< std::string > ums2AsSet(ums2.begin(), ums2.end());
        RCF_CHECK(ums1AsSet == ums2AsSet);
    }
    ...
    void testHashMap()
    {
        RCF_HASH_MAP_NS::hash_map<std::string, int> hm1, hm2;
        RCF_HASH_MAP_NS::hash_multimap<std::string, int> hmm1, hmm2;

        RCF_HASH_MAP_NS::hash_set<std::string> hs1, hs2;
        RCF_HASH_MAP_NS::hash_multiset<std::string> hms1, hms2;

        hm1["ten"] = 10;
        hm1["eleven"] = 11;

        hmm1.insert( std::make_pair("ten", 10) );
        hmm1.insert( std::make_pair("ten", 1010) );
        hmm1.insert( std::make_pair("eleven", 11) );
        ...
        std::ostringstream os;
        OStream ostr(os);
        ostr << hm1 << hmm1 << hs1 << hms1;

        std::istringstream is(os.str());
        IStream istr(is);
        istr >> hm2 >> hmm2 >> hs2 >> hms2;

        std::set< std::pair<std::string, int> > hm1AsSet(hm1.begin(), hm1.end());
        std::set< std::pair<std::string, int> > hm2AsSet(hm2.begin(), hm2.end());
        RCF_CHECK(hm1AsSet == hm2AsSet);

        std::set< std::pair<std::string, int> > hmm1AsSet(hmm1.begin(), hmm1.end());
        std::set< std::pair<std::string, int> > hmm2AsSet(hmm2.begin(), hmm2.end());
        RCF_CHECK(hmm1AsSet == hmm2AsSet);

        std::set< std::string > hs1AsSet(hs1.begin(), hs1.end());
        std::set< std::string > hs2AsSet(hs2.begin(), hs2.end());
        RCF_CHECK(hs1AsSet == hs2AsSet);

        std::multiset< std::string > hms1AsSet(hms1.begin(), hms1.end());
        std::multiset< std::string > hms2AsSet(hms2.begin(), hms2.end());
        RCF_CHECK(hms1AsSet == hms2AsSet);
    }
    ...
    void testContainers()
    {
        ...
        RCF_CHECK(s0 == s3 && s1 == s4 && s2 == s5 )(s0)(s1)(s2)(s3)(s4)(s5);
        RCF_CHECK_EQ(L2.size() , L1.size() );
        RCF_CHECK( std::equal(L1.begin(), L1.end(), L2.begin()) );
        RCF_CHECK_EQ(S2.size() , S1.size() );
        RCF_CHECK( std::equal(S1.begin(), S1.end(), S2.begin()) );
        RCF_CHECK_EQ(M2.size() , M1.size() );
        RCF_CHECK( std::equal(M1.begin(), M1.end(), M2.begin()) );
        RCF_CHECK_EQ(V2.size() , V1.size() );
        RCF_CHECK( std::equal(V1.begin(), V1.end(), V2.begin()) );
        RCF_CHECK_EQ(str2.size() , str1.size() );
        RCF_CHECK_EQ(str1 , str2 );
    ...
    void testWstring()
    {
        bool defaultNativeWcharSerialization = RCF::getDefaultNativeWstringSerialization();

        for (std::size_t i=0; i<2; ++i)
        {
            if (i==0)
            {
                RCF::setDefaultNativeWstringSerialization(false);
            }
            else
            {
                RCF::setDefaultNativeWstringSerialization(true);
            }

            std::wstring ws1 = L"\xB5m"; // micro-meter in UTF-16
            std::wstring ws2;
        ...
        RCF::setDefaultNativeWstringSerialization(defaultNativeWcharSerialization);
    }
#endif
    ...
    void testGraphs()
    {
        ...
        IStream istr(is);
        {
            ...
            RCF_CHECK_EQ(m , n);
            RCF_CHECK_EQ(pm , &m );
            RCF_CHECK_EQ(vec.size() , 6 );
            RCF_CHECK_EQ(vec[0] , vec[3] );
            RCF_CHECK_EQ(vec[1] , vec[4] );
            RCF_CHECK_EQ(vec[2] , vec[5] );
            RCF_CHECK_EQ(((A *) vec[0])->pb , ((B *) vec[1]) );
            RCF_CHECK_EQ(((B *) vec[1])->pc , ((C *) vec[2]) );
            RCF_CHECK_EQ(((C *) vec[2])->pa , ((A *) vec[0]) );

            RCF_CHECK_EQ(a2.pb->pc->pa , &a2 );
            ...
            RCF_CHECK_EQ(vec[1] , vec[4] );
            RCF_CHECK_EQ(vec[2] , vec[5] );
            RCF_CHECK_EQ(((A *) vec[0])->pb , NULL );
            RCF_CHECK_EQ(((B *) vec[1])->pc , NULL );
            RCF_CHECK_EQ(((C *) vec[2])->pa , NULL );

            RCF_CHECK_EQ(a2.pb , NULL );
    ...
    public:
        ...
        mutable RCF::Mutex      mMutex;
        mutable long            mReferenceCount;
    };

} //namespace Test_Serialization

// Making these global so that VC6 can find them.

inline void intrusive_ptr_add_ref(const Test_Serialization::RefCounted * ptr)
{
    RCF::Lock lock(ptr->mMutex);
    ++ptr->mReferenceCount;
}

inline void intrusive_ptr_release(const Test_Serialization::RefCounted * ptr)
{
    bool shouldDelete = false;
    {
        RCF::Lock lock(ptr->mMutex);
        if (0 == --ptr->mReferenceCount)
        {
            shouldDelete = true;
        }
...
namespace Test_Serialization {
    ...
    class DerivesFromRefCounted : public RefCounted
    {
    public:
        DerivesFromRefCounted() : mValue(0)
        {
        }
    ...
    template<typename IStream, typename OStream, typename IntrusivePtr>
    void testIntrusivePtr()
    {
        DerivesFromRefCounted *pn = new DerivesFromRefCounted;
        pn->mValue = 2;
        IntrusivePtr spn1(pn);
        IntrusivePtr spn2 = spn1;
        IntrusivePtr spn3 = spn2;
        IntrusivePtr spn4( (DerivesFromRefCounted *) NULL );
    ...
    void testSmartPointers()
    {
...

        {
            ...
            *pn = 1;
            ...
            RCF_CHECK_EQ(pn != NULL && pn , apn.get() );
            RCF_CHECK_EQ(apd.get() , NULL );
        }

        {

            // boost::shared_ptr
            testSharedPtr<IStream, OStream, boost::shared_ptr<int> >();

            // boost::intrusive_ptr
            testIntrusivePtr<IStream, OStream, boost::intrusive_ptr< DerivesFromRefCounted > >();
            ...
            RCF_CHECK_EQ(scpn1.get() && scpn2.get() && *scpn1 , *scpn2 );
        }
    }
    ...
    void testArchiveVersioning()
    {
...

        {
            ...
            RCF_CHECK_EQ(obs.getArchiveVersion() , 0);
    ...
    void testAll()
    {
...
#else
        testTuple<IStream,OStream, boost::tuples::tuple<int, std::string, int, std::string> >();        
#endif
        ...
        testSafeArrays<IStream, OStream, boost::array<int, 10> >();

#ifdef RCF_USE_TR1
        testTr1<IStream, OStream>();
#endif

#ifdef RCF_USE_HASH_MAP
        testHashMap<IStream, OStream>();
#endif

#ifndef BOOST_NO_STD_WSTRING
        testWstring<IStream, OStream>();
#endif
