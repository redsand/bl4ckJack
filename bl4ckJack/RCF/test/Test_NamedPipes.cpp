...
public:
    ...
    std::string echo(const std::string &s)
    {
        ...
        // Remote address is not meaningful for named pipe connections, but
        // string() should still be callable.
        const RCF::I_RemoteAddress &address = session.getRemoteAddress();
        std::string str = address.string();
