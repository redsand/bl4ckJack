...
namespace Test_Binding {
    ...
    public:
        ...
        static RCF::Mutex sCountMutex;
        static int sCount;
    };

    RCF::Mutex InstanceCounter::sCountMutex;
    int InstanceCounter::sCount = 0;

}

RCF_BROKEN_COMPILER_TYPE_TRAITS_SPECIALIZATION(Test_Binding::MyClass)
RCF_BROKEN_COMPILER_TYPE_TRAITS_SPECIALIZATION(Test_Binding::InstanceCounter)
...
namespace Test_Binding {
    ...
    public:
        ...
        InstanceCounter count(InstanceCounter ic1, const InstanceCounter & ic2, InstanceCounter & ic3)
        {
            return ic1;
        }
    };
    ...
    RCF_BEGIN(I_Echo, "I_Echo")
        ...
        RCF_METHOD_R3(InstanceCounter, count, InstanceCounter, const InstanceCounter&, InstanceCounter &)
    RCF_END(I_Echo)
    ...
    public:
        ...
        void operator()()
        {
            ...
            RCF_CHECK_EQ(&clientC1.getClientStub() , &clientC1.RcfClientB1::getClientStub());
            RCF_CHECK_EQ(&clientC1.RcfClientB1::getClientStub() , &clientC1.RcfClientC1::getClientStub());

            s = RcfClientA1(mClientTransport.clone()).funcA1();     RCF_CHECK_EQ(s , "A1");
            s = RcfClientB1(mClientTransport.clone()).funcA1();     RCF_CHECK_EQ(s , "A1");
            s = RcfClientB1(mClientTransport.clone()).funcB1();     RCF_CHECK_EQ(s , "B1");

            s = RcfClientC1(mClientTransport.clone()).funcA1();     RCF_CHECK_EQ(s , "A1");
            s = RcfClientC1(mClientTransport.clone()).funcB1();     RCF_CHECK_EQ(s , "B1");
            s = RcfClientC1(mClientTransport.clone()).funcC1();     RCF_CHECK_EQ(s , "C1");

            s = RcfClientA2(mClientTransport.clone()).funcA2();     RCF_CHECK_EQ(s , "A2");
            s = RcfClientB2(mClientTransport.clone()).funcA2();     RCF_CHECK_EQ(s , "A2");
            s = RcfClientB2(mClientTransport.clone()).funcB2();     RCF_CHECK_EQ(s , "B2");
            s = RcfClientC2(mClientTransport.clone()).funcA2();     RCF_CHECK_EQ(s , "A2");
            s = RcfClientC2(mClientTransport.clone()).funcB2();     RCF_CHECK_EQ(s , "B2");
            s = RcfClientC2(mClientTransport.clone()).funcC2();     RCF_CHECK_EQ(s , "C2");

            s = RcfClientD(mClientTransport.clone()).funcA1();      RCF_CHECK_EQ(s , "A1");
            s = RcfClientD(mClientTransport.clone()).funcA2();      RCF_CHECK_EQ(s , "A2");
            s = RcfClientD(mClientTransport.clone()).funcB1();      RCF_CHECK_EQ(s , "B1");
            s = RcfClientD(mClientTransport.clone()).funcB2();      RCF_CHECK_EQ(s , "B2");
            s = RcfClientD(mClientTransport.clone()).funcC1();      RCF_CHECK_EQ(s , "C1");
            s = RcfClientD(mClientTransport.clone()).funcC2();      RCF_CHECK_EQ(s , "C2");
            s = RcfClientD(mClientTransport.clone()).funcD();       RCF_CHECK_EQ(s , "D");

            s = RcfClientX(mClientTransport.clone()).funcA1();      RCF_CHECK_EQ(s , "A1");
            s = RcfClientX(mClientTransport.clone()).funcX();       RCF_CHECK_EQ(s , "X");

            s = RcfClientY(mClientTransport.clone()).funcA1();      RCF_CHECK_EQ(s , "A1");
            s = RcfClientY(mClientTransport.clone()).funcY();       RCF_CHECK_EQ(s , "Y");

            s = RcfClientZ(mClientTransport.clone()).RcfClientX::funcA1();  RCF_CHECK_EQ(s , "A1");
            s = RcfClientZ(mClientTransport.clone()).funcX();               RCF_CHECK_EQ(s , "X");
            s = RcfClientZ(mClientTransport.clone()).RcfClientY::funcA1();  RCF_CHECK_EQ(s , "A1");
            s = RcfClientZ(mClientTransport.clone()).funcY();               RCF_CHECK_EQ(s , "Y");
            s = RcfClientZ(mClientTransport.clone()).funcZ();               RCF_CHECK_EQ(s , "Z");

            RcfClient<E>(mClientTransport.clone()).funcE();
            std::auto_ptr<I_E> e( new RcfClient<E>(mClientTransport.clone()) );
            s = e->funcE(); RCF_CHECK_EQ(s , "E");

            s = RcfClient<F1>(mClientTransport.clone(), "F").funcF1();      RCF_CHECK_EQ(s , "F1");
            s = RcfClient<F2>(mClientTransport.clone(), "F").funcF2();      RCF_CHECK_EQ(s , "F2");
            s = RcfClient<F3>(mClientTransport.clone(), "F").funcF3();      RCF_CHECK_EQ(s , "F3");
            s = RcfClient<F4>(mClientTransport.clone(), "F").funcF4();      RCF_CHECK_EQ(s , "F4");
        }
    ...
    for (int i=0; i<RCF::getTransportFactories().size(); ++i)
    {
...

        {
...

            {
                ...
                RCF_CHECK_EQ(myClass1.mVal , val+1);
                RCF_CHECK_EQ(myClass2.mVal , val);
            }

            {
                // Check that all parameters are cleaned up after a remote call.
...

                {
                    ...
                    InstanceCounter ic2;
                    InstanceCounter ic3;
                    InstanceCounter ic4 = client.count(ic1, ic2, ic3);
                }

                RCF_CHECK_EQ(InstanceCounter::sCount , 0)(InstanceCounter::sCount);

                client.getClientStub().clearParameters();
...

        {
            ...
            s = clientC1.funcC1(); RCF_CHECK_EQ(s , "C1");
            s = clientC1.funcB1(); RCF_CHECK_EQ(s , "B1");
            s = clientC1.funcA1(); RCF_CHECK_EQ(s , "A1");
            ...
            }
            {
                ...
                s = clientB1.funcB1(); RCF_CHECK_EQ(s , "B1");
            }
        }
