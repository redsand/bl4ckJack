...
namespace Test_Filters {
    ...
    void testServerErrorHandling()
    {
        ...
        // check locking and unlocking of transport filters
        {
            ...
            {
                RCF_CHECK_OK();
                RCF_CHECK_EQ(e.getErrorId() , RCF::RcfError_FiltersLocked);
            }
        ...
        RCF::ClientTransportAutoPtr clientTransportAutoPtr)
    {
        ...
        for (unsigned int i=0; i<filterChains.size(); ++i)
        {
            for (unsigned int j=0; j<2; ++j)
            {
                ...
                RCF_CHECK_EQ(client.func(RCF::Twoway) , ++myret);
                RCF_CHECK_EQ(client.func(RCF::Twoway) , ++myret);
        ...
        const std::vector< std::vector<RCF::FilterPtr> > &filterChains)
    {
        ...
        for (unsigned int i=0; i<filterChains.size(); ++i)
        {
            ...
            RcfClient<X> client(clientTransportAutoPtr->clone());
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+1 );

            client.getClientStub().setMessageFilters(filterChain);
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+2 );
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+3 );

            client.getClientStub().setMessageFilters(std::vector<RCF::FilterPtr>());
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+4 );

            client.getClientStub().setMessageFilters(filterChain);
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+5 );
            ret = client.func(RCF::Twoway); RCF_CHECK_EQ(ret , 6*i+6 );
        }
    ...
    void testOnewayFilters(const std::vector<RCF::FilterPtr> &filterChain1, const std::vector<RCF::FilterPtr> &filterChain2)
    {
        ...
        for (int i=0; i<10; ++i)
        {
            ...
            RCF_CHECK_EQ(result , unfilteredData );
        }
...
int test_main(int argc, char **argv)
{
    ...
    RCF_CHECK_EQ(1, 0 && "Bizarre failures (crashes) with mingw 2.95.");
    return 0;
