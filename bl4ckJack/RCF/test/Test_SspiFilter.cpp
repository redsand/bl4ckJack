...
void testNamedPipeImpersonation()
{
    ...
    if (getUsernames(usernames))
    {
        ...
        RCF_CHECK_EQ(s , util::toTstring(userName));
    ...
    {
        RCF_CHECK_EQ(1, 0 && "Couldn't load usernames and passwords");
    }
}
...
int test_main(int argc, char **argv)
{
    ...
    RCF_CHECK(0 && "this test crashes on explicit-incorrect-NTLM to NTLM, with mingw 3.2 , for unknown reasons");
    return 0;
    ...
    */
    ...
    // simple warm-up test (implicit NTLM authentication)
    {
        ...
        RCF_CHECK_EQ(s , RCF::getMyUserName());
    }
            ...
            for (int k=0; k<5; ++k)
            {
                ...
                for (std::size_t j=0; j<factoriesOk.size(); ++j)
                {
...

                    {
                        ...
                        {
                            RCF_CHECK( icmp(clients[k]->getUserName(), usernameOrig));
                            RCF_CHECK(clients[k]->echo(s0) == s0 );

                            clients[k]->getClientStub().requestTransportFilters(
                                createFilter(RCF::SspiFilter::Encryption));

                            RCF_CHECK( icmp(clients[k]->getUserName(), username));
                            RCF_CHECK(clients[k]->echo(s0) == s0 );

                            clients[k]->getClientStub().clearTransportFilters();
